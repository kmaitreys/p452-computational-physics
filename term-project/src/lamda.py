...
def read(datafilepath, read_frequencies, read_quantum_numbers=False):
    ...
    for i, line in enumerate(datafile):
        ...
        if 3 + comment_offset <= i < 3 + comment_offset + n_levels:
            ...
            lev = sp.Level(
                g=leveldata[2],
                E=cc.SPEED_OF_LIGHT * cc.PLANK_CONSTANT * leveldata[1] / 1e-2,
        ...
        if i == 3 + comment_offset + n_levels:
            ...
            continue
        if (
            4 + comment_offset + n_levels
            <= i
            < 4 + comment_offset + n_levels + n_rad_transitions
        ):
            ...
            if read_frequencies:
                rad_trans_kwargs["nu0"] = radtransdata[4] * 1e9
            rad_trans = sp.RadiativeTransition(**rad_trans_kwargs)
        ...
        if (
            9 + comment_offset + n_levels + n_rad_transitions + coll_partner_offset
            <= i
            <= last_coll_trans_i
        ):
            coll_trans_data = [float(string) for string in line.split()]
            up = next(
                level for level in levels if level.number == coll_trans_data[1] - 1
            )
            low = next(
                level for level in levels if level.number == coll_trans_data[2] - 1
            )
            K21_data = np.array(coll_trans_data[3:]) * (1e-2)**3
            coll_trans = sp.CollisionalTransition(
